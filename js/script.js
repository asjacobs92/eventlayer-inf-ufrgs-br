...
var pageTags = "php/consultaTags.php"; 	//Página para a consulta dos eventos no banco de dados

$.getJSON(pageTags, tagTable);
...
function tagTable(instituteList){
	//alert(JSON.stringify(instituteList));
	for (i = 0; i < instituteList.length; i++) {
		
		var texto = "<tr>" +
				"<td class='mdl-data-table__cell--non-numeric'>" + 
					"<label class='mdl-checkbox mdl-js-checkbox mdl-js-ripple-effect' for=" + instituteList[i].instituteName +  ">" +
					"<input type='checkbox' value=" + instituteList[i].instituteId + " id=" + instituteList[i].instituteName + 
					" class='mdl-checkbox__input tag'>" + instituteList[i].instituteName + "</label>" +
				"</td></tr>";
